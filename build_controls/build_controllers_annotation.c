...
void build_controllers_annotation(User_Data *user_data)
{
	...
	/* Add motion controller to original preview. */
	...
	gtk_widget_add_controller(original_preview, GTK_EVENT_CONTROLLER(eventMouseMotion));

	/* Add gesture controller (mouse clicked) to original preview */
	...
	gtk_widget_add_controller(original_preview, GTK_EVENT_CONTROLLER(gesture_click_preview));
