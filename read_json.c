...
Settings *read_json()
{
	...
	error = NULL;
	json_parser_load_from_file(parser, "/home/abba/programming/c_programs/balloon/configuration.json", &error);
	if (error)
	{
		g_print("Unable to parse `%s': %s\n", "/home/abba/programming/c_programs/balloon/configuration.json", error->message);
		g_error_free(error);
